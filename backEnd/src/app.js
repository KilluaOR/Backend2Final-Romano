...
if (!process.env.MONGO_URI) {
  console.warn('⚠️  MONGO_URI no definida → se usa localhost. Los datos se guardan en tu MongoDB local, NO en Atlas.');
}
...
async function connectDB() {
  try {
    ...
    if (usingAtlas) {
      console.log('   → En Compass, conectate con tu connection string de Atlas y abrí la base "' + db.databaseName + '" para ver los usuarios.');
    }
  } catch (error) {
    ...
    if (error.message.includes('authentication failed') || error.message.includes('bad auth')) {
      ...
      console.error('   - En Atlas: Database Access → verificar que el usuario existe');
